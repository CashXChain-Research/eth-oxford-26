...
def run_benchmarks():
    ...
    for num_assets in asset_counts:
        ...
        try:
            ...
            logger.info(f"    ✅ Actual: {quantum_result['time']:.4f}s | Return: {quantum_result['return']:.4f} | Risk: {quantum_result['risk']:.4f}")
            results.append(BenchmarkResult(
                num_assets=num_assets,
                solver_type="quantum",
    ...
    for assets in asset_counts:
        classical = next((r for r in results if r.num_assets == assets and r.solver_type == "classical_theoretical"), None)
        quantum = next((r for r in results if r.num_assets == assets and r.solver_type == "quantum"), None)
        ...
        if classical and quantum and quantum.time_seconds != float("inf"):
            ...
            logger.info(f"{'':10} {'Quantum (actual)':<25} {quantum.time_seconds:<12.4f} {speedup:.1f}x faster")
        else:
            logger.info(f"{assets:<10} {'Error or timeout':<25}")
    
    logger.info("=" * 70)
    logger.info("\n📊 Interpretation:")
    logger.info("  - 5 assets: Classical ~0.01s | Quantum ~0.8s (classical still faster, but quantum stable)")
...
def run_benchmarks():
    ...
    for num_assets in asset_counts:
        ...
        try:
            ...
            logger.info(f"    ✅ {classical_result['time']:.4f}s | Return: {classical_result['return']:.4f} | Risk: {classical_result['risk']:.4f}")
            results.append(BenchmarkResult(
                num_assets=num_assets,
        ...
        try:
            quantum_result = solve_quantum_annealing(mu, cov, risk_tolerance=0.5)
            logger.info(f"    ✅ {quantum_result['time']:.4f}s | Return: {quantum_result['return']:.4f} | Risk: {quantum_result['risk']:.4f}")
            results.append(BenchmarkResult(
                num_assets=num_assets,
                solver_type="quantum",
                time_seconds=quantum_result["time"],
    ...
    for assets in asset_counts:
        classical = next((r for r in results if r.num_assets == assets and r.solver_type == "classical"), None)
        quantum = next((r for r in results if r.num_assets == assets and r.solver_type == "quantum"), None)
        ...
        if classical and quantum and classical.time_seconds != float("inf"):
            ...
            logger.info(f"{'':10} {'Quantum':<15} {quantum.time_seconds:<12.4f} {speedup:.1f}x faster")
        else:
